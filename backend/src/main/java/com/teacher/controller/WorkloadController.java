...
public class WorkloadController {
    ...
    public ResponseEntity<?> approveWorkload(@PathVariable Long id) {
        try {
            return ResponseEntity.ok(workloadService.approveWorkload(id));
        } catch (IllegalArgumentException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(Map.of("message", e.getMessage()));
        }
    ...
    public ResponseEntity<?> rejectWorkload(@PathVariable Long id, @RequestBody WorkloadAuditRequest request) {
        try {
            return ResponseEntity.ok(workloadService.rejectWorkload(id, request.getRejectReason()));
        } catch (IllegalArgumentException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(Map.of("message", e.getMessage()));
        }
